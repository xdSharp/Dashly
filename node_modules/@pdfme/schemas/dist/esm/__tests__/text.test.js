...
import { calculateDynamicFontSize, getBrowserVerticalFontAdjustments, getFontDescentInPt, getFontKitFont, getSplittedLines, filterStartJP, filterEndJP, } from '../src/text/helper.js';
...
describe('filterStartJP', () => {
    ...
    test('すべての禁則文字を正しく処理する', () => {
        const input = LINE_START_FORBIDDEN_CHARS.map((char) => ['この', char + '文字']).flat();
        const expected = LINE_START_FORBIDDEN_CHARS.map((char) => [
            'この' + char,
            '文字',
        ]).flat();
        expect(filterStartJP(input)).toEqual(expected);
...
describe('filterEndJP', () => {
    ...
    test('すべての禁則文字を正しく処理する', () => {
        const input = LINE_END_FORBIDDEN_CHARS.map((char) => ['これは' + char, '文章']).flat();
        const expected = LINE_END_FORBIDDEN_CHARS.map((char) => [
            'これは',
            char + '文章',
        ]).flat();
        expect(filterEndJP(input)).toEqual(expected);
